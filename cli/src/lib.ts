...
export async function getEnv(): Promise<"stg" | "prod" | "dev"> {
  if (
    ...
    if (hostname === "prod-vm") {
        ...
        throw new Error(`Unable to determine env: Hostname ${hostname} does not match {prod|stg}-vm`);
    }
  }

  const repoRootFile = await findUpwardFile(process.cwd(), ".veracioux-website-repo-root");

  if (repoRootFile) {
    return "dev";
  }
  
  throw new Error("Please run this command in the veracioux/website repository or on a prod/stg VM.");
}
