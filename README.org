...
- *Terraform* as the IaC tool (see [[file:infra/main.tf][infra/main.tf]])
- *Ansible* for provisioning (see [[file:infra/playbook.yml][infra/playbook.yml]])
- *Vagrant* for preview deployment in a local VM (see [[file:infra/Vagrantfile][infra/Vagrantfile]])
...
- Provision a temporary staging environment via [[file:infra/playbook.yml][Ansible]]
- Clone production data to a temporary staging instance
- (New version was already deployed in the)
- Run tests on staging
- If tests pass, deploy to production via [[file:infra/playbook.yml][Ansible provisioning]]
* Configuration
** Local ports
