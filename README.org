...
(+ [[compose.prod-merge.yml]] for production-specific overrides). The following
services exist:
...
following tools:
- *Docker Compose* for orchestrating the services (see [[file:compose.yml][compose.yml]] and [[file:compose.prod-merge.yml][compose.prod-merge.yml]])
